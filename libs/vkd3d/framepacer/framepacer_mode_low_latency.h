...
namespace dxvk {
  ...
  public:
    ...
    LowLatencyMode(Mode mode, LatencyMarkersStorage* storage, FrameSync* frameSync, uint64_t firstFrameId, int refreshRate = 0)
    : FramePacerMode(mode, mode == LOW_LATENCY ? "low-latency" : "low-latency-vrr", storage, frameSync, firstFrameId),
      m_lowLatencyOffset(0),//getLowLatencyOffset(options)),
      m_allowCpuFramesOverlap(true),//options.lowLatencyAllowCpuFramesOverlap),
    ...
    void startFrame( uint64_t frameId ) override {
      ...
      m_frameSync->m_fenceGpuStart.wait( frameId-1, 200 );
      ...
      if (targetGpuTime < props.optimizedGpuTime) {
        ...
        m_frameSync->m_fenceGpuFinished.wait( frameId-1, 200 );
      }
