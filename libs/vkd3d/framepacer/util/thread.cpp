...
namespace dxvk {
  ...
  : m_data(new ThreadData(std::move(proc))) {
    ...
    if (!m_data->handle) {
      delete m_data;
      abort_with_error( "Failed to create thread \n" );
  ...
  void thread::join() {
    ...
    if(::WaitForSingleObjectEx(m_data->handle, INFINITE, FALSE) == WAIT_FAILED)
      abort_with_error( "Joining thread failed \n" );
